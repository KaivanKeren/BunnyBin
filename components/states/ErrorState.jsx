...
export default function ErrorState({ correct, item, onTryAgain }) {
  const isOrganic = correct === "organic";
  const lesson = isOrganic
    ? {
        title: "Ini Sampah Organik!",
        text: "Sampah organik berasal dari makhluk hidup seperti sisa makanan, daun, atau kulit buah. Bisa diolah menjadi kompos!",
        emoji: "🍃",
      }
    : {
        title: "Ini Sampah Anorganik!",
        text: "Sampah anorganik seperti plastik, kaleng, dan kertas dapat didaur ulang menjadi barang baru. Ayo pilah dengan benar!",
        emoji: "♻️",
      };
  ...
  return (
    <motion.div
      ...
      style={{
        background:
          "linear-gradient(135deg, #FFE5E5 0%, #FFB8B8 60%, #FF8A8A 100%)",
      }}
    >
      {/* Red pulsing glow */}
      <motion.div
      ...
      >
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-red-600">{lesson.title}</h2>
            <p className="mt-1 text-sm leading-relaxed text-[#6b5b50]">
              {lesson.text}
            </p>
            {item && (
              <div className="mt-2 inline-block rounded-full bg-red-100 px-3 py-1 text-xs font-semibold text-red-600">
                Contoh: {item.emoji} {item.name}
              </div>
            )}
          </div>
        </div>
