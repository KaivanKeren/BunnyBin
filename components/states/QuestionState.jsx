...
export default function QuestionState({ onAnswer, item, disabledChoice }) {
  return (
    <motion.div
      key="question"
      initial={{ opacity: 0 }}
    ...
    >
      ...
      >
        ...
        {item && (
          <>
            ...
            <span className="text-lg font-bold text-[#3a2f29]">{item.name}</span>
          </>
        )}
      </motion.div>

      {/* Bunny + question AFTER detection */}
      <div className="flex items-center gap-5">
        <Bunny mood="think" size={150} />
        <motion.div
        ...
        >
          <div className="text-xs font-semibold text-[#9b8a7c]">
            Bunny bertanya...
          </div>
          <div className="text-2xl font-bold text-[#3a2f29]">
            Menurutmu, ini sampah apa?
          </div>
      ...
      <div className="grid w-full max-w-3xl grid-cols-2 gap-6">
        <ChoiceButton
          type="organic"
          disabled={disabledChoice === "organic"}
          onClick={() => onAnswer("organic")}
        />
        <ChoiceButton
          type="inorganic"
          disabled={disabledChoice === "inorganic"}
...
function ChoiceButton({ type, disabled, onClick }) {
  ...
  if (disabled) {
    return (
      <motion.div
        ...
        className="relative flex flex-col items-center gap-3 rounded-3xl border-4 border-gray-300 bg-gray-100 px-6 py-8 opacity-60"
      >
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.97 }}
      onClick={onClick}
      className={`group flex flex-col items-center gap-3 rounded-3xl border-4 ${cfg.border} ${cfg.bg} px-6 py-8 shadow-pop transition ${cfg.bgHover}`}
    >
      <div
        className={`rounded-2xl ${cfg.iconBg} p-4 text-white shadow-soft ${cfg.iconBgHover}`}
      >
        <cfg.Icon size={48} />
      </div>
